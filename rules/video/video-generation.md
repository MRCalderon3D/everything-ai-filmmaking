...
- Lens language is provider-gated (`scripts/lib/lens-language.js`): Veo-class
  providers take lens/focus tokens at the head of the prompt; keyframe-driven
  providers like Runway Gen-4 get motion-only prompts — their vendor guide
...
- Models overact by default: prompts MUST carry playable behavior from the
  shot's `performance` block, never emotion adjectives ("terrified"), and
  declared intensity is written contained-or-below to photograph believable
  (`skills/performance-direction`). A clip whose acting reads cartoon is a
  prompt defect, not a model mood.
...
- **Rerolls change exactly ONE variable** (seed, one prompt clause, one
  reference, or one parameter) — otherwise a fixed clip teaches you nothing
  about what fixed it. Diagnose failures in order: subject → action →
  camera → style → audio → output; most failures live in the first two.
- Providers without negative-prompt syntax (Seedance-class) get prevention
  phrased POSITIVELY inside the main prompt ("all limbs naturally
  positioned"), never as appended negations; the compiler drops
  `negative_prompt` for them and flags it in the record.
