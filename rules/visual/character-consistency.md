...
- **Approved masters outrank bible text.** New assets are checked against
  the approved master IMAGES first, the bible's words second — including
  the rendered EXTENT of marks (a wound's spread, wear, staining), not just
  their side and presence. Where image and text disagree, the image is
  canon and the bible is stale: sync it (`common/approval-policy.md`)
  before generating anything that would inherit the discrepancy.
- **Laterality protocol.** Mirror confusion is the most common visual audit
  error — in humans and in model self-review alike. Anatomical sides are
  derived BEFORE judging any asymmetric anchor (earrings, wounds, scars,
  torn sleeves): in a profile or three-quarter view, the visible side is the
  side the nose points toward in viewer terms. Findings name sides
  anatomically ("her right ear"), never by frame side.
- Asymmetric anchors are verified at zoom on the actual region, never from a
  thumbnail; an anchor judged at contact-sheet size counts as unverified.
